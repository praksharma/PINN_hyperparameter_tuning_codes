...
def run(cfg: ModulusConfig) -> None:
    ...
    output_keys=[Key("T")] # always the temperature field
...
#     heat_domain.add_validator(openfoam_validator)
    ...
    data = np.load(f'{sweep_specific_directory}/inferencers/inf_data.npz',allow_pickle=True)
    ...
    first_file = np.load(to_absolute_path('true_solution/final_data_1.01.npz'))
    second_file = np.load(to_absolute_path('true_solution/final_data_1.03.npz'))
    third_file = np.load(to_absolute_path('true_solution/final_data_1.05.npz'))
    ...
    #logging.info(f"Data 1: {rel_l2_norm_1}')
    # K= 0.5
    indices = return_indices(lower_limit_k = 1.021, upper_limit_k=1.031, k=k_array)
    new_data = griddata(nodes_deepxde[indices], best_u_pred[indices].flatten(), (X, Y), method='linear')
    new_data_1 = griddata(nodes_second, second_true.flatten(), (X, Y), method='linear')
    ...
    #logging.info("Data 2:",rel_l2_norm_2)
    ...
    #logging.info("Data 3:",rel_l2_norm_3)

    average_L2_error = (rel_l2_norm_1 + rel_l2_norm_2 + rel_l2_norm_3)/3
    #logging.info("Average",average_L2_error)
        
    rel_l2_norm_of_error = average_L2_error
